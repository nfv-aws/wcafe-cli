...
func newStoresUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update <store_id>",
		Short: "Update a store",
		RunE:  runStoresUpdateCmd,
	}
	return cmd
}
...
// stores updateの出力
func runStoresUpdateCmd(cmd *cobra.Command, args []string) error {
	// store_idが指定されているか確認
	if len(args) == 0 {
	...
	body, err := client.StoreUpdate(ctx, store_id)
	if err != nil {
		return errors.Wrapf(err, "StoreUpdate was failed:body = %+v", body)
	}
	fmt.Println(body)
...
func (client *Client) StoreUpdate(ctx context.Context, store_id string) (string, error) {
